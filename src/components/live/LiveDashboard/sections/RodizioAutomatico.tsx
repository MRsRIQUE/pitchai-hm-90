...
/**
 * Rodízio automático — a extensão troca sozinha o produto fixado na live.
 *
 * Mora na seção Live, e não no catálogo: isto não é cadastro de produto, é
 * regra de transmissão. Quem abre o Produtos vai gerenciar a vitrine; quem
 * liga o rodízio está preparando o que vai acontecer no ar. O componente lê
 * tudo da store, então serve em qualquer seção sem prop nenhuma.
 */
export function RodizioAutomatico() {
  ...
  const updateAutoFixar = (value: LiveConfig["autoFixar"]) => {
    ...
    void pushLiveConfigFields({ autoFixar: value }).catch((error) => {
      console.error("[RodizioAutomatico] falha ao sincronizar Auto-Fixar:", error);
  ...
  return (
    <div className="app-section">
      <div className="app-card">
        ...
        {/* Os ajustes só aparecem com o rodízio ligado: desligado, os três
            campos e a lista inteira são decoração ocupando a tela da live. */}
        {config.autoFixar.enabled ? (
          <>
            ...
            <div className="app-card app-card--flat" style={{ marginTop: 16 }}>
              <div className="app-card-head" style={{ marginBottom: 10 }}>
                <h3 className="app-section-title">Produtos no rodízio</h3>
              ...
              {config.produtos.length === 0 ? (
                <p className="app-field-hint">
                  Traga a vitrine na seção Produtos para escolher o que entra no rodízio.
                </p>
              ) : visiveis.length === 0 ? (
