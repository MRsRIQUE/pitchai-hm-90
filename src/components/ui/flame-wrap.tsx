...
export interface FlameWrapOptions {
  ...
  scorch?: number;
  /**
   * Captura o conteúdo para dentro do canvas (HTML-in-canvas), o que liga a
   * distorção de calor e o derretimento sobre ele. Passe `false` quando o
   * conteúdo precisa continuar sendo DOM normal — dentro do canvas ele sai do
   * fluxo do layout, não dita mais a altura do pai e o texto passa pelo
   * shimmer do shader. O fogo em volta é desenhado do mesmo jeito.
   */
  capture?: boolean;
}
...
export function createFlameWrap(
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        // a captura de HTML em canvas é experimental: se falhar num quadro,
        // o efeito continua desenhando o fogo sem o conteúdo
      }
    };
  }
  ...
  function syncCanvasSize() {
    ...
    if (outRect.width > 0 && boxRect.width > 0) {
      /* getBoundingClientRect já vem com o transform dos ancestrais aplicado,
         mas o canvas é dimensionado em px de layout (clientWidth). Sem dividir
         pela escala, um scale() no wrapper amplia e desloca o retângulo do
         fogo em relação ao conteúdo. */
      const sx = outRect.width / Math.max(output.clientWidth, 1);
      const sy = outRect.height / Math.max(output.clientHeight, 1);
      rect.cx = ((boxRect.left + boxRect.right) / 2 - outRect.left) / sx;
      rect.cy = (outRect.bottom - (boxRect.top + boxRect.bottom) / 2) / sy;
      rect.hx = boxRect.width / 2 / sx;
      rect.hy = boxRect.height / 2 / sy;
    }
    if (htmlInCanvas) {
...
export function FlameWrap({ children, className, style, ...options }: FlameWrapProps) {
  ...
  const detected = useSyncExternalStore(emptySubscribe, supportsHtmlInCanvas, () => false);
  const native = options.capture !== false && detected && !failed;
        ...
        {native ? (
          <div
            ...
            style={{
              ...
              width: "100%",
              height: "100%",
              overflow: options.capture === false ? "visible" : "auto",
            }}
          >
            {children}
          </div>
